...
#define POWER_SUPPLY_ACPATH "/sys/class/power_supply/ac/online"
#define POWER_SUPPLY_USBPATH "/sys/class/power_supply/usb/online"
...
static int getBatteryStatus(const char* status) {
	switch (status[0]) {
        ...
        default: {
                     ...
                     return gConstants.statusUnknown;       /* Unknow */
                 }
	}
}
...
static int getBatteryHealth(const char* status) {
	switch (status[0]) {
		...
		if (strcmp(status, "Overheat") == 0) {
			return gConstants.healthOverheat;    /* Overheat */
		} else if (strcmp(status, "Over voltage") == 0) {
                        return gConstants.healthOverVoltage; /* Overvoltage */
                }
		LOGW("Unknown battery health[1] '%s'", status);
                return gConstants.healthUnknown;         /* Unknow */
        }

        case 'U': {
                if (strcmp(status, "Unspecified failure") == 0) {
                        return gConstants.healthUnspecifiedFailure;/* Unspecifiedfailure */
...
int battery_status_init(void) {
	...
	if (access(POWER_SUPPLY_ACPATH, R_OK) == 0)
		gPaths.acOnlinePath = strdup(POWER_SUPPLY_ACPATH);
	if (access(POWER_SUPPLY_USBPATH, R_OK) == 0)
		gPaths.usbOnlinePath = strdup(POWER_SUPPLY_USBPATH);
	...
	while ((entry = readdir(dir))) {
		...
		if (length > 0) {
			...
			if (!gPaths.acOnlinePath && strcmp(buf, "Mains") == 0) {
				snprintf(path, sizeof(path), "%s/%s/online",
					 POWER_SUPPLY_PATH, name);
				if (access(path, R_OK) == 0)
					gPaths.acOnlinePath = strdup(path);
			} else if (!gPaths.usbOnlinePath && strcmp(buf, "USB") == 0) {
				snprintf(path, sizeof(path), "%s/%s/online",
					 POWER_SUPPLY_PATH, name);
				if (access(path, R_OK) == 0)
				...
				snprintf(path, sizeof(path), "%s/%s/health",
					 POWER_SUPPLY_PATH, name);
				if (access(path, R_OK) == 0)
					gPaths.batteryHealthPath = strdup(path);
				snprintf(path, sizeof(path), "%s/%s/present",
					 POWER_SUPPLY_PATH, name);
				if (access(path, R_OK) == 0)
				...
				snprintf(path, sizeof(path), "%s/%s/voltage_now",
					 POWER_SUPPLY_PATH, name);
				if (access(path, R_OK) == 0) {
				...
				// voltage_now is in microvolts, not millivolts
					...
					snprintf(path, sizeof(path), "%s/%s/batt_vol",
						 POWER_SUPPLY_PATH, name);
					if (access(path, R_OK) == 0)
						gPaths.batteryVoltagePath = strdup(path);
				}

				snprintf(path, sizeof(path), "%s/%s/temp",
					 POWER_SUPPLY_PATH, name);
				if (access(path, R_OK) == 0) {
					gPaths.batteryTemperaturePath = strdup(path);
				} else {
					snprintf(path, sizeof(path), "%s/%s/batt_temp",
						 POWER_SUPPLY_PATH, name);
					if (access(path, R_OK) == 0)
						gPaths.batteryTemperaturePath = strdup(path);
				}
	...
	if (!gPaths.acOnlinePath || !gPaths.usbOnlinePath) {
		LOGE("ac or usb OnlinePath not found");
		return -1;
	}
	...
	if (!gPaths.batteryPresentPath || !gPaths.batteryCapacityPath) {
		LOGE("battery PresentPath or CapacityPath not found");
		return -1;
	}
	...
	gConstants.healthCold = BATTERY_HEALTH_COLD;
...
int battery_health(void)
{
	...
	if (!gPaths.batteryHealthPath) {
		LOGE("gPaths.batteryHealthPath not found");
		if (battery_status_init())
			return -1;
	}
        pthread_mutex_lock(&gBatteryMutex);
        if (readFromFile(gPaths.batteryHealthPath, buf, SIZE) > 0)
                ...
                setInt(mBatteryHealth, gConstants.healthUnknown);

        ret = PowerSupplyStatus[mBatteryHealth];
